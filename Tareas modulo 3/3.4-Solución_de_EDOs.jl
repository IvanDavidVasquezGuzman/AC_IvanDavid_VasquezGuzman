...
function tamañoDePaso(t0, tf, N)
		# Calculamos el valor del tamaño de paso de acuerdo a la definición
		h = (tf-t0)/N
	# Pedimos que nos regrese el tamaño de paso.
	return h
end

# ╔═╡ 18f2d50c-967d-4476-ab91-900ec840d247
# Probamos la función.
tamañoDePaso(10,20,10000)
...
# Definimos la función con sus parámetros
function arregloUniforme(t0,N,h)
		# Calculamos el tf
		tf = t0 + N*h
		# Establecemos la condición de que la función se realice en caso de que 'N' sea mayor a cero.
		if N > 0
			# Calculamos el arreglo
			tiempos = range(t0,tf ,step = h)
		else 
			# En caso de que 'N' sea menor o igual a cero debe imprimir un mensaje de error.
			print("N no es un entero positivo")
		end 
	return tiempos
end

# ╔═╡ 8ed95442-c16b-48f9-884a-127998143b71
begin
arregloUniforme(2,1000,0.001)
end

# ╔═╡ 30177c73-a930-42b6-9d46-6180ea65b010
# Probamos que funcione la condición 
arregloUniforme(2,-1000,0.001)
...
# Definimos la función con sus parámetros
function paso_euler(g, ti, xti, h)
		# Calculamos el valor aproximado de x_i+1 de acuerdo a la definición
		aprox = xti + h*g(ti, xti)
	return aprox
end

# ╔═╡ 723c35be-e995-421d-b7e7-b1768e742fe3
# Definimos una ecuación diferencial de prueba.
function ecdif(t,y)
	return cos(t)
end

# ╔═╡ fdb3d9a5-1554-4227-b481-b385d5b3a550
md""" Para la ecuación diferencial propuesta tenemos que:

$$\frac{dy}{dt} \cos(t)$$
$$y(0) = \pi/2$$
$$y(t) = \sin(t) + \pi/2$$

Por lo que la ecuación para $t(0) = 0$ es $\frac{\pi}{2}$ 
"""

# ╔═╡ 1ed8c38d-4454-4d58-9523-ef0b49931078
# Probamos nuestra función 'paso_euler' con la ecuación diferencial y el valor inicial mencionados anteriormente.
paso_euler(ecdif, 0, pi/2, 0.001)
...
# Definimos la función euler con sus parametros.
function euler(g,x0,t) 
	# Hacemos una variable con el tamaño del arreglo t.
	N = length(t)
	# Hacemos un arreglo de ceros del mismo tamaño que t, que almacenará los valores del valor aproximado de la solución de la ecuación diferencial en los puntos t_i.
	val_aprox = zeros(N)
	# El valor inicial o condición inicial será x0.
	val_aprox[1] = x0
	# Hacemos un ciclo 'for' que calcule las soluciones al tiempo t_i de acuerdo a la definición.
	for i in range(1,N-1,step = 1)
		val_aprox[i+1] = val_aprox[i] + 0.0001*g(t[i], val_aprox[i])
	end
	# Pedimos que nos regrese el arreglo de valores aproximados.
	return val_aprox
end
...
**La integral de la función con respecto de t y con x(0) (la condición inicial), como constante de integración, como vimos en la función de prueba de la función **paso_euler**.**
...
# Definimos una función g sencilla para probar la función 'euler'.
function gsencilla(t,x)
	return t
end

# ╔═╡ 669fb6f9-3025-443b-a3e2-01fb922c1828
md""" Para la ecuación diferencial propuesta tenemos que:
...
# ╔═╡ 65a926a3-d318-45b5-b011-36b16acadc7a
begin
# Hacemos un arreglo uniforme de tiempo donde probar nuestra función.
t = arregloUniforme(0,30000,0.0001)
# Calculamos el arreglo de soluciones.
ye = euler(gsencilla,0,t)
# Graficamos el tiempo con las soluciones.
plot(t, ye)
end

# ╔═╡ 60cb60ec-e30b-4d19-9712-59904251a1d1
begin
# Hacemos un arreglo uniforme de tiempos para probar la función ecdif mencionada anteriormente y graficamos de igual manera que en la celda de código anterior.
tp = arregloUniforme(0,31415,0.0001)
yp = euler(ecdif,pi/2,tp)
plot(tp,yp)
end
...
# ╔═╡ 9514d4cc-01d4-4c4d-bda2-f54e25d92a25
# Hacemos un deslizador con los valores de N.
deslizadorN = @bind N Slider(range(1,60000, step = 100))

# ╔═╡ 7c3e0db7-f751-472a-b5a4-6cf8554ef41c
# Imprimimos el valor de N.
N

# ╔═╡ 7d7979e7-8301-47e5-a4e2-53e32da199da
# Definimos la Solución del ejemplo como una ecuación diferencial.
function SolucionD(t,x)
	return 5*exp(t)
end

# ╔═╡ c7473ba5-186d-4820-ba32-50492975a62c
begin
# Definimos la solución analítica encontrada en el ejemplo.
SolucionAnalitica(t) = 5*exp(t)
# Hacemos un arreglo uniforme de tiempo.
tiempo = arregloUniforme(-1, N, 0.0001)
# Calculamos la solución con nuestra función 'euler'
Solucion = euler(SolucionD,5,tiempo)
# Graficamos la solución obtenida con nuestra función y el tiempo. 
plot(tiempo, Solucion, label = "Solución con Método de Euler")
# Graficamos la solución analítica con el tiempo.
plot!(tiempo,SolucionAnalitica, label = "Solución analítica")
end
