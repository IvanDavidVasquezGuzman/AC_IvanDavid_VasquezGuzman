...
# ╔═╡ 432eb817-41b5-43ba-8aaf-c2bc353637eb
...
# Definimos la función con sus respectivos parámetros.
function newton(f,f′,x0,n)
	# Hacemos un arreglo de ceros de tamaño 'n' que 'almacene' los valores de x.
	x = zeros(n)
	# Tomamos como valor inicial 'x0'. 
	x[1] = x0
	# Hacemos un ciclo 'for' que calcule los valores de 'x' hasta 'x_n'
	for i in range(1,n-1, step=1)
		# Implementamos la definición de x_i+1.
		x[i+1] = x[i] - f(x[i])/f′(x[i]) 
	end
	# Pedimos que nos regrese el 'n-esimo' valor del arreglo de x's que es la solución.
	return x[n]
end

# ╔═╡ 40dc3586-9cbe-4ba1-bb12-82b7993a37f2
# Utilizamos la función para encontrar el cero de una función conocida (con valor inicial 2 por lo que el valor más próximo en que la función es 0 es en pi).
newton(sin,cos,2.5, 10000)

# ╔═╡ c29c9202-8939-4c19-94f2-b6a7d1b5f9e2
# Definimos otra función para probar 'newton' y verificar su funcionamiento
begin
# Definimos una función arbitraria en la que sabemos cuando es cero.
function fprueba(x)
	return x-2
end
...
function newtonRecursiva(f,f′,x0,n)
	...
	if n == 1
	   ...
	   x = x0 
		...
		# Hacemos un ciclo recursivo
	   x = newtonRecursiva(f,f′,x0,n-1) - f(newtonRecursiva(f,f′,x0,n-1))/f′(newtonRecursiva(f,f′,x0,n-1)) 
	end
	# Pedimos a la función que nos regrese el valor de x cuando la función es 0.
return x
end

# ╔═╡ 6acb9cbc-89a7-462f-b769-0b567447799e
# Probamos nuestra función
newtonRecursiva(sin,cos,2,10)
...
function newtonDistancia(f, f′,x0, ε)
	# Tomamos un valor inicial de 'f0' y 'dis', donde 'dis' es el valor absoluto de la distancia de dos aproximaciones consecutivas
	f0 , dis = 2*ε , 2*ε
	# Hacemos un condicional que cumpla que se realice la función si el valor de ε es positivo.
	if ε > 0 
	# Hacemos un ciclo while cuya condición sea que el ciclo continue hasta que la distancia sea menor o igual al valor de ε
	while dis > ε
		# Hacemos otra condición que sea que el ciclo continue hasta que el valor de f0 sea menor o igual a ε
		while abs(f0) > ε
	# Asignamos un valor inicial a x0 (x0 comienzo)
	x0c = x0
	# Calculamos el valor de la función en x0
	f0 = f(x0)
	# Calculamos el nuevo valor de x0 con el que continuará el ciclo
	x0 = x0 - f(x0)/f′(x0)
	# Calculamos la distancia entre las aproximaciones consecutivas.
	dis = abs(x0 - x0c)
		end
end
	# Si ε es negativo o igual a cero debe imprimir un mensaje de error.
	else
		# Ponemos el mensaje de error.
		print("ERROR, ε es menor o igual a cero.")
	
end
	# Pedimos que nos regrese la solución.
	return x0
end

# ╔═╡ 8f95c4a8-6198-4076-9b47-c6ce0673d621
# Probamos que nuestra función 'newtonDistancia' funcione.
newtonDistancia(sin,cos,2,1e-6)

# ╔═╡ 04990e60-ad8b-41b8-8949-10d9b7663899
# Probamos que se cumpla la condición al usar un valor negativo de ε. 
newtonDistancia(sin,cos,2,-1e-6)
...
function derivadaNumérica(f,x,h)
	# Calculamos el valor aproximado de la derivda numérica en 'x' de acuerdo a la definición 
aproxf′ = h*(f(x+1/h)-f(x))
	# Pedimos que nos regrese el valor aproximado 
	return aproxf′
end

# ╔═╡ 8bf85857-00f6-4ef1-9c17-5e6d71b07b81
# Probamos nuestra función 'derivadaNumérica' de una función donde sabemos su derivada y el valor de esta última en x (cos(0)=1).
derivadaNumérica(sin,0,10000)
...
function newtonDN(f,x0,n,h)
	# Hacemos un arreglo de ceros de tamaño 'n' que 'almacene' los valores de x. 
	x = zeros(n)
	# Hacemos un arreglo de ceros de tamaño 'n' que 'almacene' los valores de f′.
	f′ = zeros(n)
	# Tomamos com valor inicial 'x0'.
	x[1] = x0
	# Hacemos un ciclo 'for' que calcule los valores de 'x' hasta 'x_n'
	for i in range(1,n-1, step=1)
		# Implementamos la definición de la derivada numérica.
		f′[i] = h*(f(x[i]+1/h)-f(x[i]))
		# Implementamos la definición de x_i+1 usando la derivada numérica.
		x[i+1] = x[i] - f(x[i])/f′[i] 
	end
	# Pedimos que nos regrese el 'n-esimo' valor del arreglo de x's que es la solución.
	return x[n]
...
# ╔═╡ 5fb85eea-2285-4b86-9ead-37253ae28812
# Probamos nuestra función 'newtonDN'
newtonDN(sin,2.5, 1000,1000)
...
# ╔═╡ c60c7152-0687-4b67-864d-10857b262133
# Hacemos los deslizadores de las variables.
deslizador1 = @bind x0 Slider(-10:0.1:10, default = 1)

# ╔═╡ 2dd84acc-510a-4d11-9975-f1c1ce94c9ba
# Mostramos el valor de las varibles
x0

# ╔═╡ 6f334c71-9a8a-4e54-8435-4eb5af8c5a69
deslizador2 = @bind h Slider(1000:1000:1000000, default = 1)

# ╔═╡ daa98aad-c309-47d0-9951-faa164bd0a7a
h

# ╔═╡ b1603442-6b17-4608-b899-df9551159419
deslizador3 = @bind n Slider(1:1:1000, default = 1)
